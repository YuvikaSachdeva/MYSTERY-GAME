...
function setup() {
  createCanvas(displayWidth,displayHeight);
      ...
      border21=createSprite(900,656,100,45);
   ...
   dentalclick= createSprite(994,183,65,50)
   dentalclick.visible=false;
   woodclick=createSprite(1192,300,60,20);
   woodclick.visible=false;
   dipclick=createSprite(445,65,115,50);
   dipclick.visible=false;
   xclick=createSprite(733,90,120,50);
   xclick.visible=false;
   sclick=createSprite(204,200,80,25);
   sclick.visible=false;
   Mclick=createSprite(1080,268,60,80);
   Mclick.visible=false;
   Hclick=createSprite(558,20,70,60);
   Hclick.visible=false;
   hiclick=createSprite(640,175,50,40);
   hiclick.visible=false;
   glovesclick=createSprite(445,200,55,40);
   glovesclick.visible=false;
...
function draw() {
 ...
 text("DEVELOPED BY: YUVIKA SACHDEVA", displayWidth/2-280,displayHeight-290)
  ...
  if(gameState===4){
    ...
    //changing the image to tickmark if mouse pressed 
    
    if(mousePressedOver(xclick)){
      border13.addImage("xray",tickmark)
      //text("YOU FOUND IT!!",displayWidth/2-50,displayHeight-20)
      flag14=true
    }

    if(mousePressedOver(sclick)){
      border14.addImage("spectacles",tickmark)
      //text("WELL DONE!",displayWidth/2-50,displayHeight-20)
      flag15=true

    }

    if(mousePressedOver(Mclick)){
      border15.addImage("microscope",tickmark)
      flag16=true

    }

    if(mousePressedOver(Hclick)){
      border16.addImage("humanskull",tickmark)
      flag17=true

    }

    if(mousePressedOver(hiclick)){
      border17.addImage("hilighter",tickmark)
      flag18=true

    }

    if(mousePressedOver(glovesclick)){
      border18.addImage("gloves",tickmark)
      flag19=true

    }

    if(mousePressedOver(dipclick)){
      border19.addImage("diploma",tickmark)
      flag20=true

    }

    if(mousePressedOver(dentalclick)){
      border20.addImage("dentaltool",tickmark)
      flag21=true

    }

    if(mousePressedOver(woodclick)){
      border21.addImage("wood",tickmark)
      flag22=true

    }

   
    if(flag14===true&& flag15===true && flag16===true && flag17===true && flag18===true && flag19===true&& flag20===true && flag21===true && flag22===true){
      textSize(80);
      fill(221,160,221);
      text("FANTASTIC!!",displayWidth/2-250,displayHeight/2-200);
  ...
  if(gameState===5){
    ...
    textFont('chiller');
      textSize(70)
      fill(0)
      text("YOU HAVE COMPLETED THE MYSTERY GAME!!",displayWidth/2-530,displayHeight/2+90)
    if(frameCount%20==0){
